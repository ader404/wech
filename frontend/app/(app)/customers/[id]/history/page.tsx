...
import { ArrowLeft, ShoppingCart, DollarSign, CreditCard, RefreshCw, FileText } from 'lucide-react'
...
export default function CustomerHistoryPage({ params }: { params: { id: string } }) {
  ...
  const { data: history, isLoading } = useQuery<CustomerHistory>({
    queryKey: ['customer-history', params.id, startDate, endDate],
    queryFn: () => api.get(`/customers/${params.id}/history`, {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col flex-1">
        <Header title={t('history.header.title')} />
        <div className="flex-1 flex items-center justify-center">
          <p className="text-muted-foreground">{t('history.header.loading')}</p>
        </div>
      </div>
    )
  }
  ...
  if (!history) {
    return (
      <div className="flex flex-col flex-1">
        <Header title={t('history.header.title')} />
        <div className="flex-1 flex items-center justify-center">
          <p className="text-muted-foreground">{t('history.header.notFound')}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold">{history.customer.name}</h2>
              <p className="text-sm text-muted-foreground">{t('history.header.subtitle')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">{t('history.summary.totalSales')}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{formatCurrency(history.summary.totalSales)}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">{t('history.summary.totalRefunds')}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{formatCurrency(history.summary.totalRefunds)}</div>
            </CardContent>
          </Card>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="text-right">
                          <p className={`text-lg font-bold ${
                            ...
                            {activity.type === 'PAYMENT' || activity.type === 'LOAN_PAYMENT' || activity.type === 'REFUND' ? '-' : ''}
